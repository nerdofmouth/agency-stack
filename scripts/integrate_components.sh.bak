...
main() {
  ...
  log "${BLUE}Available Integrations:${NC}"
  log "1. Email Integration with Mailu"
  log "2. Single Sign-On with Keycloak"
  log "3. Monitoring Integration with Loki/Grafana"
  log "4. Data Exchange Bridge"
  log "5. Run All Integrations"
  log ""
  
  # Ask for integration choice if not in auto mode
  if [ "$AUTO_MODE" = false ]; then
    read -p "Select integration to configure (1-5): " choice
  else
    # Run all integrations in auto mode
    choice=5
  fi
