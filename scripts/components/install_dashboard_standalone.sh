...
EOF
  ...
  # Create wildcard route for anyhost
  WILDCARD_ROUTE="${TRAEFIK_DYNAMIC_DIR}/wildcard-guaranteed.yml"
  
  cat > "${WILDCARD_ROUTE}" <<EOF
http:
  routers:
    # Wildcard router to catch all domains
    wildcard-router:
      rule: "PathPrefix(\`/\`)"
      entrypoints:
        - "web"
...
if [ -f "${TRAEFIK_MAIN_CONFIG}" ]; then
  ...
  # Remove all redirections to force HTTP access
  sed -i '/redirections/d' "${TRAEFIK_MAIN_CONFIG}" 
  sed -i '/entryPoint/d' "${TRAEFIK_MAIN_CONFIG}"
  sed -i '/scheme/d' "${TRAEFIK_MAIN_CONFIG}"
...
HTTP_PATH_STATUS=$(curl -s -o /dev/null -w "%{http_code}" "http://${DOMAIN}/dashboard" || echo "Failed")
log_info "HTTP status for http://${DOMAIN}/dashboard: ${HTTP_PATH_STATUS}"

DIRECT_STATUS=$(curl -s -o /dev/null -w "%{http_code}" "http://${SERVER_IP}:${DASHBOARD_PORT}" || echo "Failed")
log_info "HTTP status for http://${SERVER_IP}:${DASHBOARD_PORT}: ${DIRECT_STATUS}"

# Create Nginx configuration for direct access as last resort
if [ "${NGINX_AVAILABLE}" = "true" ]; then
  log_info "Setting up Nginx for direct access..."
...
EOF
  ...
  if [ ! -L "/etc/nginx/sites-enabled/dashboard" ]; then
    ln -s "${NGINX_CONF}" "/etc/nginx/sites-enabled/dashboard"
  fi
