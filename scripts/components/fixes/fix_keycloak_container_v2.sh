...
# Create new docker-compose file with correct YAML formatting
...
services:
  postgres:
    ...
    container_name: keycloak_postgres_DOMAIN_PLACEHOLDER
    restart: unless-stopped
    volumes:
      - KEYCLOAK_DIR_PLACEHOLDER/DOMAIN_PLACEHOLDER/postgres-data:/var/lib/postgresql/data
    environment:
  ...
  keycloak:
    ...
    container_name: keycloak_DOMAIN_PLACEHOLDER
    restart: unless-stopped
    ...
    environment:
      ...
      KC_DB_PASSWORD: DB_PASSWORD_PLACEHOLDER
      KEYCLOAK_ADMIN: admin
      KEYCLOAK_ADMIN_PASSWORD: ADMIN_PASSWORD_PLACEHOLDER
      KC_PROXY: edge
      KC_HOSTNAME_URL: https://DOMAIN_PLACEHOLDER/auth
      KC_HOSTNAME_ADMIN_URL: https://DOMAIN_PLACEHOLDER/auth
    volumes:
      - KEYCLOAK_DIR_PLACEHOLDER/DOMAIN_PLACEHOLDER/keycloak-data:/opt/keycloak/data
      - KEYCLOAK_DIR_PLACEHOLDER/DOMAIN_PLACEHOLDER/themes:/opt/keycloak/themes
      - KEYCLOAK_DIR_PLACEHOLDER/DOMAIN_PLACEHOLDER/imports:/opt/keycloak/imports
    depends_on:
      - postgres
    networks:
      - agency-network
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.keycloak_DOMAIN_PLACEHOLDER.rule=Host(`DOMAIN_PLACEHOLDER`)"
      - "traefik.http.routers.keycloak_DOMAIN_PLACEHOLDER.entrypoints=websecure"
      - "traefik.http.routers.keycloak_DOMAIN_PLACEHOLDER.tls.certresolver=myresolver"
      - "traefik.http.services.keycloak_DOMAIN_PLACEHOLDER.loadbalancer.server.port=8080"
      - "traefik.http.middlewares.keycloak_DOMAIN_PLACEHOLDER_security.headers.browserXssFilter=true"
      - "traefik.http.middlewares.keycloak_DOMAIN_PLACEHOLDER_security.headers.contentTypeNosniff=true"
      - "traefik.http.middlewares.keycloak_DOMAIN_PLACEHOLDER_security.headers.forceSTSHeader=true"
      - "traefik.http.middlewares.keycloak_DOMAIN_PLACEHOLDER_security.headers.stsIncludeSubdomains=true"
      - "traefik.http.middlewares.keycloak_DOMAIN_PLACEHOLDER_security.headers.stsPreload=true"
      - "traefik.http.middlewares.keycloak_DOMAIN_PLACEHOLDER_security.headers.stsSeconds=31536000"
      - "traefik.http.routers.keycloak_DOMAIN_PLACEHOLDER.middlewares=keycloak_DOMAIN_PLACEHOLDER_security"
...
log_info "Configuring docker-compose.yml with instance-specific values"
sed -i "s|DOMAIN_PLACEHOLDER|${DOMAIN}|g" "$COMPOSE_FILE"
sed -i "s|KEYCLOAK_DIR_PLACEHOLDER|${KEYCLOAK_DIR}|g" "$COMPOSE_FILE"
sed -i "s|DB_PASSWORD_PLACEHOLDER|${DB_PASSWORD}|g" "$COMPOSE_FILE"
sed -i "s|ADMIN_PASSWORD_PLACEHOLDER|${ADMIN_PASSWORD}|g" "$COMPOSE_FILE"
