#!/bin/bash
# Launchbox Installer Script
# https://nerdofmouth.com/launchbox
# This script helps manage the installation of the Launchbox components
...
STATEFILE="/opt/launchbox/installation_state.json"
mkdir -p "/opt/launchbox" || { echo "Failed to create /opt/launchbox directory. Please run with sudo."; exit 1; }
...
███████╗██║  ██║╚██████╔╝██║ ╚████║╚██████╗██║  ██║██████╔╝╚██████╔╝██╔╝ ██╗
╚══════╝╚═╝  ╚═╝ ╚═════╝ ╚═╝  ╚═══╝ ╚═════╝╚═╝  ╚═╝╚═════╝  ╚═════╝ ╚═╝  ╚═╝
               Open Source Freedom Stack for Agencies & Enterprises
               https://nerdofmouth.com/launchbox
EOF
echo ""
echo -e "${BLUE}${BOLD}Launchbox Installation & Management Center${NC}"
echo -e "${CYAN}Logging to: $LOGFILE${NC}"
echo ""

log "INFO" "Starting Launchbox installer"

# Installation state tracking
STATEDIR="/home/revelationx/.launchbox"
...
if [ ! -f "/opt/launchbox/config.env" ]; then
  ...
  print_header "Launchbox Initial Configuration"
  ...
  # Save configuration to file with Launchbox branding
  cat > /opt/launchbox/config.env << ENVFILE
# Launchbox Configuration
# Generated by Launchbox Installer
# https://nerdofmouth.com/launchbox
...
STACK_VERSION=1.0.0
STACK_BRAND_URL=nerdofmouth.com
...
COMPOSE_PROJECT_NAME=launchbox
...
ENVFILE
  ...
  log "INFO" "Initial configuration saved to /opt/launchbox/config.env"
  print_success "Configuration saved to /opt/launchbox/config.env"
  
  # Create version file
  echo "1.0.0" > /opt/launchbox/version
  
  # Create component directory
  mkdir -p /opt/launchbox/clients
  mkdir -p /var/log/launchbox
  
  # Create a .clients.json file for tracking clients
  echo '{"clients":[]}' > /opt/launchbox/clients/.clients.json
...
install_component() {
  ...
  case $component in
    ...
    40) 
      echo -e "${BLUE}${BOLD} Installing Launchbox Core Components...${NC}"
    ...
    50) 
      ...
      log "INFO" "Installing all Launchbox components"
      ;;
    60) 
      if [ -f "$PORT_MANAGER" ]; then
      ...
      ;;
    70)
      echo -e "${CYAN}${BOLD} Launchbox System Status:${NC}"
      ...
      ;;
    80)
      echo -e "${CYAN}${BOLD} Launchbox Installation Log:${NC}"
      if [ -f "$LOGFILE" ]; then
  ...
  echo -e "\n${GREEN}${BOLD} Installing Launchbox component: $script ${NC}"
  ...
  if [ -f "$SCRIPT_DIR/$script" ]; then
    ...
    if [ $exit_code -eq 0 ]; then
      ...
      echo -e "${GREEN}${BOLD} Launchbox component $script installed successfully${NC}"
      # Record successful installation
      echo "$(date +"%Y-%m-%d %H:%M:%S") - $script" >> /opt/launchbox/installed_components.txt
      ...
      echo -e "${RED}${BOLD} Launchbox component $script installation failed${NC}"
      return 1
    fi
  else
    update_state "$script" "failed"
...
if [ ! -f "/usr/local/bin/agency-stack" ]; then
  log "INFO" "Creating agency-stack management symlink"
  cat > /usr/local/bin/agency-stack << 'AGENCYSCRIPT'
#!/bin/bash
...
case "$1" in
  ...
  install)
    ...
    ;;
  help|*)
    echo "Usage: agency-stack [command]"
    echo "Commands:"
...
AGENCYSCRIPT
  chmod +x /usr/local/bin/agency-stack
  ...
  log "INFO" "Created agency-stack command for easier management"
  echo -e "${GREEN}Created 'agency-stack' command for easier management${NC}"
fi
...
main_menu() {
  ...
  while true; do
    ...
    if [ "$choice" -eq 0 ]; then
      ...
      echo -e "${GREEN}You can manage your installation with the 'agency-stack' command${NC}"
      exit 0
    else
